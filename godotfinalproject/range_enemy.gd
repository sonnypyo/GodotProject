...
func _process(delta):
	...
	if Input.is_action_just_pressed("ui_accept"):  # 스페이스바 기본 바인딩 키
		...
		var enemy_current_pos = global_position
		var current_pos = Vector2i(
		int(enemy_current_pos.x / TILE_SIZE),
		int(enemy_current_pos.y / TILE_SIZE)
		)
		print("ddddddddddddddddddddddddddddddddd")
		print(current_pos)
		if (current_pos + Vector2i(2, 0) == target_pos or current_pos + Vector2i(2, -1) == target_pos or current_pos + Vector2i(2, 1) == target_pos or 
		current_pos + Vector2i(-2, 0) == target_pos or current_pos + Vector2i(-2, 1) == target_pos or current_pos + Vector2i(-2, -1) == target_pos or 
